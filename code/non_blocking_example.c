...
char **argv;
{
    ...
    if (rank == 0) {
        ...
        scanf("%d", &buffer[0]);
        MPI_Isend(&buffer[0], count, MPI_INT, destination, tag, MPI_COMM_WORLD, &request); //non blocking send to destination process
    }

    if (rank == destination) {
        MPI_Irecv(&buffer2[0], count, MPI_INT, 0, tag, MPI_COMM_WORLD, &request); //destination process receives
    }
